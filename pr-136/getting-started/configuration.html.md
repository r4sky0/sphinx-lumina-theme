...
| `pagefind_executable` | string | `""`         | Optional path to the Pagefind executable. When empty, Lumina checks `PATH` and then uses `npx --no-install pagefind`. |
| `pagefind_timeout`    | string | `"30"`       | Maximum number of seconds allowed for Pagefind indexing before the build logs a timeout and continues.                |
