...
const isIcons = process.argv.includes("--icons");
...
function buildIcons({ force = false } = {}) {
  ...
  if (!existsSync(iconsDir)) {
    console.warn("lucide-static icons not found in node_modules, skipping icon build.");
    return;
  }
  ...
  if (existsSync(outFile)) {
    ...
    if (existing === python) {
      console.log(`Icons up to date: ${files.length} icons.`);
      return;
    }
  }
...
if (isIcons) {
  buildIcons({ force: true });
} else {
  buildIcons({ force: false });
  buildJS();
  buildCSS();
}
